...
8. **Mandatory Categorical Reason Codes for Escalation**: Required every `escalate=True` decision to emit a validated categorical reason code (`account_security_credentials`, `billing_payment_dispute`, `physical_hardware_safety`, `legal_threat`, `channel_transition`, `unsupported_device_scope`). A black-box boolean flag offers zero auditability or routing utility.
